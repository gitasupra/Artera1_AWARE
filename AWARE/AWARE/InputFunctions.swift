...
class InputFunctions : ObservableObject{
    ...
    func processData(windowFile: String) {
        ...
        DispatchQueue.global().async {
            ...
            do {
                testfile = try Bundle.main.path(forResource: "BK7610", ofType: "csv")!
            } catch {
                print("Error: \(error.localizedDescription)")
            ...
            for metricNum in Features.allCases {
                let perSecondDataFile = self.create_per_second_data(file: testfile, metric_no: metricNum.rawValue)
                perWindowDataDir = self.create_per_window_data(file: perSecondDataFile, metric_no: metricNum.rawValue)
            }
            ...
            self.combine_features(csvPath: perWindowDataDir)
